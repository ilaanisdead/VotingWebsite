    ...
    <?php
        if(isset($_POST["SearchValue"])) {
            ...
            try {
                ...
                $Db_Command = $conn->prepare("SELECT * FROM admin WHERE id LIKE '%$search%' OR username LIKE
                '%$search%' OR email LIKE '%$search%'
                
                ");
                $Db_Command->execute();

                $Db_Command2 = $conn->prepare("SELECT COUNT(*) FROM admin WHERE id LIKE '%$search%' OR username LIKE
                '%$search%' OR email LIKE '%$search%'
