...
<nav class="navbar navbar-expand-lg bg-dark border-bottom border-body" data-bs-theme="dark">
    ...
    session_start();
        ...
        if(isset($_GET['Uid'])){
            ...
            $Db_Command = $conn->prepare("SELECT * FROM post WHERE id='$id'");
    ...
    ?>
        ...
        <form class="form_css container-fluid py-4" action="EditPost.php" method="post">
            <input type="hidden" class="form-control" name= "id2" value="<?php echo $row['id']; ?>">
            
            <p>
                <label class="form-label" for="title">Title</label>
                <input name="title" value="<?php echo $row["title"] ?>" class="form-control" id="title" type="text" required>
            </p>

            <p>
                <label class="form-label" for="desc">Description</label>
                <input name="desc" value="<?php echo $row["Description"] ?>" class="form-control" id="desc" type="text" required>
            </p>
...
<?php
    ...
    if(isset($_POST['Update'])){
            ...
            $DBb_Command = "UPDATE post SET title='$title',Description='$desc' WHERE id='$id2'";

            $result3 = $conn->exec($DBb_Command);
            if($result3){
                header("Location:ShowPosts.php?status=Edited&Msg=Success");
            }
            else{
                echo "failed to Update :(. Please try again";
                ?>
                <a href="EditPost.php?Uid=<?php echo $_SESSION['SUid'] ?>">Back</a>
                <?php
            }
