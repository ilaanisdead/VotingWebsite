    ...
    <?php
        if(isset($_POST["SearchValue"])) {
            ...
            try {
                ...
                $Db_Command = $conn->prepare("SELECT * FROM course WHERE id LIKE '%$search%' OR Course_Name LIKE
                '%$search%' OR Department LIKE '%$search%'
                
                ");
                $Db_Command->execute();

                $Db_Command2 = $conn->prepare("SELECT COUNT(*) FROM course WHERE id LIKE '%$search%' OR Course_Name LIKE
                '%$search%' OR Department LIKE '%$search%'
